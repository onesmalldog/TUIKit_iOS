...
class AnchorSeatListView: UIView {
    ...
    private var seatViews: [AnchorSeatItemView] = []
    ...
    private func setupViews() {
        ...
        for i in 0..<seatCount {
            ...
            seatInfo.index = i
            let seatView = AnchorSeatItemView(seatInfo: seatInfo) { [weak self] info in
                self?.onTapSeat?(info)
            }
...
// MARK: - AnchorSeatItemView

class AnchorSeatItemView: UIView {
    ...
    func update(seatInfo: SeatInfo) {
        ...
        if isEmpty {
            placeholderImageView.image = internalImage("live_seat_empty_icon")
            nameContainer.isHidden = true
        } else {
