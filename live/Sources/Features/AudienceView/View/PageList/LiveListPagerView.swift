...
extension LiveListPagerView: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if let liveView = cell.contentView.subviews.first,
           (indexPath.row != 0 || currentPage != 0) {
    ...
    func tableView(_ tableView: UITableView, didEndDisplaying cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        guard let liveView = cell.contentView.subviews.first else { return }
        
        if deletingIndexPaths.remove(indexPath) != nil {
            return
        }
        
        if indexPath.row == confirmedPage {
            delegate?.onViewDidSlideOut(view: liveView)
        } else {
            delegate?.onViewSlideInCancelled(view: liveView)
        }
    }
}
...
extension LiveListPagerView: UIScrollViewDelegate {
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        ...
        if newPage != currentPage {
            let newIndexPath = IndexPath(row: newPage, section: 0)
    ...
    func scrollViewDidEndScrollingAnimation(_ scrollView: UIScrollView) {
        ...
        if nextPage < liveList.count {
            ...
            deletingIndexPaths.insert(oldIndexPath)
